'use client';

import { useGetCalls } from '@/hooks/useGetCalls';
import { Call, CallRecording } from '@stream-io/video-react-sdk';
import { useRouter } from 'next/navigation';
import React from 'react';
import MeetingCard from './MeetingCard';
import Loader from './Loader';
import { useToast } from './ui/use-toast';

const CallList = ({ type }: { type: 'ended' | 'upcoming' | 'recordings' }) => {
	const { endedCalls, upcomingCalls, callRecordings, isLoading } = useGetCalls();
	const { push } = useRouter();
	const [recordings, setRecordings] = React.useState<CallRecording[]>([]);
	const { toast } = useToast();

	const getCalls = () => {
		switch (type) {
			case 'ended':
				return endedCalls;
			case 'recordings':
				return recordings;
			case 'upcoming':
				return upcomingCalls;
			default:
				return [];
		}
	};

	const getNoCallsMessage = () => {
		switch (type) {
			case 'ended':
				return 'No Previous Calls';
			case 'recordings':
				return 'No Recordings';
			case 'upcoming':
				return 'No Upcoming Calls';
			default:
				return '';
		}
	};

	React.useEffect(() => {
		const fetchRecordings = async () => {
			try {
				const callData = await Promise.all(callRecordings.map((meeting) => meeting.queryRecordings()));

				const recordings = callData
					.filter((call: { recordings: string | any[] }) => call.recordings.length > 0)
					.flatMap((call: { recordings: any }) => call.recordings);

				setRecordings(recordings);
			} catch (error) {
        toast({ title: 'Please, try again later 😥' })
      }
		};

		if (type === 'recordings') fetchRecordings();
	}, [type, callRecordings]);

	const calls = getCalls();
	const noNoCallsMessage = getNoCallsMessage();

	if (isLoading) return <Loader />;

	return (
		<div className='grid grid-cols-1 gap-5 xl:grid-cols-2'>
			{calls && calls.length > 0 ? (
				calls.map((meeting: Call | CallRecording) => (
					<MeetingCard
						key={(meeting as Call)?.id}
						icon={
							type === 'ended'
								? '/assets/icons/previous.svg'
								: type === 'upcoming'
								? '/assets/icons/upcoming.svg'
								: 'assets/icons/recordings.svg'
						}
						title={
							(meeting as Call).state?.custom?.description ||
							(meeting as CallRecording)?.filename?.substring(0, 20) ||
							'Personal Meetinghome '
						}
						date={
							(meeting as Call).state?.startsAt!.toLocaleString()
						}
            startTime={(meeting as CallRecording).start_time.toLocaleString()}
            endTime={(meeting as CallRecording).end_time.toLocaleString()}
						isPreviousMeeting={type === 'ended'}
						link={
							type === 'recordings'
								? (meeting as CallRecording).url
								: `${process.env.NEXT_PUBLIC_BASE_URL}/meeting/${(meeting as Call).id}`
						}
						buttonText={type === 'recordings' ? 'Play' : 'Start'}
						buttonIcon={type === 'recordings' ? '/assets/icons/play.svg' : undefined}
						handleClick={
							type === 'recordings'
								? () => push(`${(meeting as CallRecording).url}`)
								: () => push(`/meeting/${(meeting as Call).id}`)
						}
					/>
				))
			) : (
				<h1>{noNoCallsMessage}</h1>
			)}
		</div>
	);
};

export default CallList;
